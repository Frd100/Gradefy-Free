...
struct EvaluationProvider: TimelineProvider {
    func placeholder(in context: Context) -> EvaluationEntry {
        EvaluationEntry(
            date: Date(),
            hasEvaluation: true,
    ...
    func getSnapshot(in context: Context, completion: @escaping (EvaluationEntry) -> Void) {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<EvaluationEntry>) -> Void) {
...
// MARK: - Vue Router Principale MODIFIÉE POUR PREMIUM
struct EvaluationWidgetView: View {
    var entry: EvaluationEntry
    @Environment(\.widgetFamily) var family

    var body: some View {
        // ✅ SEULE MODIFICATION : Vérification premium
        if WidgetPremiumHelper.isPremiumUser() {
            // Votre design original inchangé
            switch family {
            case .systemSmall:
            ...
            // ✅ Vue verrouillée pour non-premium
            switch family {
            case .accessoryCircular, .accessoryRectangular, .accessoryInline:
                WidgetLockedAccessoryView()
            default:
                WidgetLockedView(widgetTitle: String(localized: "widget_evaluations_locked"))
            }
        }
    }
}
