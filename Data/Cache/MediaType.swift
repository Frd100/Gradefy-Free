...
enum MediaCacheExpiration {
    ...
    var timeInterval: TimeInterval {
        switch self {
        case let .images(days): return TimeInterval(days * 24 * 60 * 60)
        case let .audio(days): return TimeInterval(days * 24 * 60 * 60)
        case let .thumbnails(days): return TimeInterval(days * 24 * 60 * 60)
        case let .compressed(days): return TimeInterval(days * 24 * 60 * 60)
        }
    }
}
...
class MediaCacheManager: ObservableObject {
    ...
    private init() {
        ...
        configuration = MediaCacheConfiguration.configuration(for: Int(deviceMemory))

        // Setup disk cache
        let cacheDirectory = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("GradefyMediaCache", isDirectory: true)
        diskCache = MediaDiskCache(cacheDirectory: cacheDirectory, maxSize: configuration.diskCacheLimit)
