...
class DeckSharingManager: ObservableObject {
    ...
    private func getCreatorName(context: NSManagedObjectContext) -> String? {
    ...
    func importDeck(from data: Data, context: NSManagedObjectContext, limitToFreeQuota: Bool = false) async throws -> FlashcardDeck {
        ...
        return try await context.perform {
            ...
            // Déterminer le nombre de cartes à importer selon les limites globales
            let maxGlobal = PremiumManager.shared.isPremium ? 2000 : 300
            let currentFlashcards = try context.count(for: NSFetchRequest<Flashcard>(entityName: "Flashcard"))
            let remainingCapacity = max(0, maxGlobal - currentFlashcards)
            
            let cardLimit = limitToFreeQuota ? 
                min(remainingCapacity, shareableDeck.flashcards.count) : 
                shareableDeck.flashcards.count
    ...
    func importDeckDirect(shareableDeck: ShareableDeck, context: NSManagedObjectContext, limitToFreeQuota: Bool = false) async throws -> FlashcardDeck {
        return try await context.perform {
            ...
            print("📊 Import de \(cardLimit) cartes sur \(shareableDeck.flashcards.count) disponibles (capacité restante: \(remainingCapacity))")
